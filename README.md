...
The enrichment pass added exact-IP Tier B recipes for Mullvad (also Mozilla/Firefox VPN infrastructure), IVPN, Private Internet Access, AirVPN, Windscribe, PrivadoVPN, RiseupVPN, NordVPN (opt-in heavy), and VPN Gate (opt-in public relays). It also added opt-in DNS-expanded recipes for provider-published hostnames from Surfshark, IPVanish, PrivateVPN, PureVPN, TorGuard, FastestVPN, VPNSecure, TunnelBear, and VPNBook. Apple Private Relay stays `relay`, and Cloudflare ranges stay context-only. Still-open research candidates include ProtonVPN official alternatives, ExpressVPN, CyberGhost, Perfect Privacy, Kape/Pango white-label brands, Cloudflare WARP-specific egress, and other provider APIs that expose exact IPs without client impersonation. The standard is source quality first, provider coverage second.
